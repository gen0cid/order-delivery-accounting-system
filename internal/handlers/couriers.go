...
func (h *CourierHandler) SetReviewToCourier(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		writeErrorResponse(w, http.StatusMethodNotAllowed, "Method not allowed")
		return
	}
	...
	courierId, err := extractUUIDFromPath(r.URL.Path, "/api/CourierReview/")
	if err != nil {
		writeErrorResponse(w, http.StatusBadRequest, "Invalid courier ID")
		return
	}

	FeedbackResponse, err := h.courierService.SetRating(feedback, courierId, feedback.OrderId)
	if err != nil {
		writeErrorResponse(w, http.StatusInternalServerError, "Failed to set rating")
		return
	}

	cacheKey := redis.GenerateKey(redis.KeyPrefixCourier, courierId.String())
	if err := h.redisClient.Set(r.Context(), cacheKey, FeedbackResponse, defaultCacheTTL); err != nil {
		h.log.WithError(err).Error("Failed to cache feedback")
		// Не возвращаем ошибку клиенту - кэш не критичен
	}

	h.log.WithField("courier_id", FeedbackResponse.CourierId).Info("review created successfully")
	writeJSONResponse(w, http.StatusOK, FeedbackResponse)
