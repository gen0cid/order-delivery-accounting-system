...
func (s *CourierService) CreateCourier(req *models.CreateCourierRequest) (*models.Courier, error) {
	...
	query := `
		INSERT INTO couriers (id, name, phone, status, created_at, updated_at, current_lat, current_lon)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
	`

	_, err := s.db.Exec(query, courier.ID, courier.Name, courier.Phone,
		courier.Status, courier.CreatedAt, courier.UpdatedAt, courier.CurrentLat, courier.CurrentLon)
	if err != nil {
	...
	s.log.WithFields(map[string]interface{}{
		...
		"phone":        courier.Phone,
		"current_lat":  courier.CurrentLat,
		"current_lon":  courier.CurrentLon,
	}).Info("Courier created successfully")

	return courier, nil
}

// SetRating создаем рейтинг у курьера
